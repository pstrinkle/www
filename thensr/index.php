...
<div class="post"><p class="yellow" style="text-align: left; padding-left: 10px;">
<?
# /home/content/p/s/t/pstrink2/html/
$messages = $root . 'data/nsr/post.xml';

if ($rp = fopen($messages, "r")) {
	$file = file_get_contents($messages);
}

fclose($rp);

if (preg_match_all('/<author>(.*?)<\/author>\s*<message>(.*?)<\/message>\s*<date>(.*?)<\/date>/', $file, $messages)) {
	for ($i = 0; $i < sizeof($messages[1]); $i++) {
		print $messages[3][$i] . " :: " . $messages[1][$i] . " :: " . htmlspecialchars($messages[2][$i]) . "<br>\n";
	}
}
?>
</p></div>
